 ...
 * @param {string} [locale] Optional locale code for language-specific handling.
 * @returns {string} The slugified string.
 */
function slugify(text, fallback = '', locale = null) {
    ...
    try {
        ...
        if (!slug) {
            console.log('⚠️ [slugify] Empty slug generated, using fallback:', fallback);
            return fallback;
        }
        console.log('🔤 [slugify] Generated slug:', slug, locale ? `(locale: ${locale})` : '');
        return slug;
    } catch (error) {
        console.error('❌ [slugify] Error generating slug:', error.message);
        return fallback;
    }
}
...
async function createAndProcessTranslation({ articleId, language, originalArticle }) {
    ...
    try {
        ...
        const translatedSlug = slugify(translatedHeadline, articleId, language); // Generate slug from translated headline with locale
